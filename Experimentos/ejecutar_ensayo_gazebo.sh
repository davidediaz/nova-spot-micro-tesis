...
campaign_dir="${CAMPAIGN_DIR:-campana_ppo_gazebo_20260902}"
out="$root/Experimentos/$campaign_dir/$label"
...
sleep 5
ros2 topic pub -r 5 /nova/gait_command std_msgs/msg/String '{data: paso}' >"$recdir/start.log" 2>&1 & starter=$!
sleep 8; kill "$starter" 2>/dev/null || true
sleep "${WAIT_SECONDS:-60}"
ros2 topic pub -r 5 /nova/gait_command std_msgs/msg/String '{data: stand}' >"$recdir/stop.log" 2>&1 & stopper=$!
sleep 8; kill "$stopper" 2>/dev/null || true
