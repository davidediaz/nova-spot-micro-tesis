...
def main():
    ...
    nominal=[rows(BASE/f'nominal_0{i}') for i in (1,2,4,5)]
    ...
    for field,label in FIELDS:
        ...
        result.append({'metrica':label,'nominal_media':n.mean(),'ppo_reentrenada_media':q.mean(),
                       'diferencia_media':d,'diferencia_relativa_pct':100*d/n.mean()})
    ...
    for axis,field,label in [(ax[0],'pitch_max_abs_deg','Pitch máximo (°)'),(ax[1],'avance_m','Avance (m/ciclo)')]:
        ...
        axis.plot(np.arange(len(n))+1,n,'o-',label='Nominal válido'); axis.plot(np.arange(len(q))+1,q,'s--',label='PPO reentrenada')
    ...
    lines=['# Revalidación dinámica nominal–PPO en Gazebo','','- Cuatro ensayos nominales válidos y cinco PPO; `nominal_03` se excluyó por cadencia inválida.','- PPO: política reentrenada con contactos, IMU y altura corporal; semilla 11.','- La comparación es descriptiva y no emparejada.','', '| Métrica | Nominal válido | PPO reentrenada | Diferencia relativa |','|---|---:|---:|---:|']
    for r in result: lines.append(f"| {r['metrica']} | {r['nominal_media']:.6f} | {r['ppo_reentrenada_media']:.6f} | {r['diferencia_relativa_pct']:.3f} % |")
